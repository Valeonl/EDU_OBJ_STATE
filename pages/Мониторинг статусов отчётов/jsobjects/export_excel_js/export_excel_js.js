export default {
	async export_excel() {
		try {
			...
			const ou_type = appsmith.store.download_type
			const report_ou_type = ou_type === 'private' 
			? "Частные" 
			: ou_type === 'gov' 
			? "Государственные" 
			: "Все ОУ";
			...
			const tableConfigs = [
				...
				{ 
					...
					include_in_report: ou_type === 'gov' ? false : true,
					headers: [
						{
							"id": "iogv_short_name",
							"label": "iogv_short_name",
			...
			const commonTitle = `Расшифровка (${report_ou_type}). Мониторинг по вопросу обеспеченности учебниками образовательных учреждений\n${formatAgencyName(appsmith.store.superuser.agencyname)}`;
			...
			const fileName = `Расшифровка (${report_ou_type}). Мониторинг по вопросу обеспеченности учебниками образовательных учреждений ${formatAgencyName(appsmith.store.superuser.agencyname)}_${currentDate}.xlsx`;
