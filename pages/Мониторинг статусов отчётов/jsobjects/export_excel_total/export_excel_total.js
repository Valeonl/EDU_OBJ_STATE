		...
		try {
			...
			const tableConfigs = [
				{ 
					...
					totals: {
						enabled: true,
						sumColumns: ['textbooks_in_stock', 'additional_textbook_demand'],
						totalRowStyle: {
				...
				{ 
					...
					totals: {
						enabled: true,
						sumColumns: ['textbooks_in_stock', 'additional_demand'],
						totalRowStyle: {
							fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'F2F2F2' } },
							font: { bold: true }
						}
			...
			const commonTitle = `Отчёт (${report_ou_type}). Мониторинг по вопросу обеспеченности учебниками образовательных учреждений\n${formatAgencyName(appsmith.store.superuser.agencyname)}`;
			...
			const fileName = `Отчёт (${report_ou_type}). Мониторинг по вопросу обеспеченности учебниками образовательных учреждений ${formatAgencyName(appsmith.store.superuser.agencyname)}_${currentDate}.xlsx`;
