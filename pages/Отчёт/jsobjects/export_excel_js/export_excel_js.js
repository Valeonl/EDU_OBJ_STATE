export default {
	async export_excel() {
		try {
			const is_private_ou = appsmith.store.user.is_private_ou;
			...
			try {
				const queries = [
					{ query: form_1_sql_data, name: "форме 1" },
					{ query: form_2_1_sql_data, name: "форме 2.1" },
					{ query: form_2_2_sql_data, name: "форме 2.2" },
					{ query: form_3_sql_data, name: "форме 3" },
					{ query: form_4_sql_data, name: "форме 4" },
					{ query: form_5_1_sql_data, name: "форме 5.1" },
					{ query: form_5_2_sql_data, name: "форме 5.2" }
				];
			...
			const tableConfigs = [
				...
				{ 
					table: form_2_1_sql_data.data,
					include_in_report: is_private_ou,
					headers: [
						{
							"id": "iogv_short_name",
							"label": "iogv_short_name",
			...
			const inn = appsmith.store.user.login;
			const currentDate = new Date().toISOString().split('T')[0];
			const workbook = new ExcelJS.Workbook();


			// Общий заголовок для всех форм
			const commonTitle = `Расшифровка. Мониторинг по вопросу обеспеченности учебниками образовательных учреждений\n${appsmith.store.user.agencyname}`;
			...
			const addTableToSheet = (worksheet, config, isFirstSheet) => {
				...
				if (isFirstSheet) {
					...
					// Устанавливаем высоту строки с учетом переноса текста
					titleRow.height = 50; // Начальное значение, которое будет автоматически подстраиваться
					...
					// Подстраиваем высоту строки под содержимое
					const textLines = commonTitle.split('\n').length;
					const lineHeight = 20; // Примерная высота одной строки в пунктах
			...
			// Фильтруем конфигурации, включая все формы где include_in_report не false
			tableConfigs
				.filter(config => config.include_in_report !== false)
			...
			const fileName = `Расшифровка (${appsmith.store.user.login}). Мониторинг по вопросу обеспеченности учебниками образовательных учреждений ${appsmith.store.user.agencyname}_${currentDate}.xlsx`;
